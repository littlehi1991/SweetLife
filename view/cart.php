    ...
    <body>
        ...
        <main>
            ...
            <div class="container">
                ...
                if(isset($cartNum)) {
                        foreach ($cartNum as $k => $v) {            //將存在session的購物清單回圈取出鍵跟值
                            foreach ($val as $k0 => $v0) {
                                if ((int)$v0['id'] === (int)$v[0]) {  //如果產品id===session儲存的pid
                                    ...
                                    <div class="card-body">
                                    <img src="<?php echo '../controller/' . $v0['main_img']; ?>" alt="產品文字"
                                         width="50%;">
                                ...
                                <div class="form-group">
                                    ...
                                    <select onchange="getId( <?= $v0['id'] ?>)"
                                            class="form-control period_select_<?php echo $v0['id']; ?>" id="period"
                                            name="period">
                                        <?php
                                        foreach ($pval as $k2 => $v2) {
                                            if ((int)$v[2] === (int)$v2['period']) { ?>
                                                <option value="<?php echo $v2['period']; ?>"
                                                        selected><?php echo $v2['period']; ?></option>
                                            <?php } else { ?>
                                                <option
                                                    value="<?php echo $v2['period']; ?>"><?php echo $v2['period']; ?></option>
                                            <?php }
                                        } ?>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="exampleFormControlTextarea" rows="3"
                                              name="others"><?php echo $v[3]; ?></textarea>
                                </div>
                                </form>
                                <a href="<?php echo DOMAIN . "SweetsLife/view/programinside.php?id=" . $v[0]; ?>">
                                    <button type="button" class="btn btn-primary">查看商品</button>
                                </a>
                                <a href="<?php echo DOMAIN . "SweetsLife/controller/removeCart.php?id=" . $v[0]; ?>"
                                   onclick="return confirm('確定移除？')">
                                    <button type="button" class="btn btn btn-secondary">移除商品</button>
                                </a>
                                </div>
                                </div>
                            <?php }
